...
**Что кешем не покрыто.** Список `GET /events` с произвольными фильтрами и пагинацией — у него слишком много вариантов ключа при небольшой выгоде, он ходит в базу напрямую. Кеша на запись (write-through) тоже нет: `POST /events` не прогревает `event:{id}` заранее — новый ключ появится при первом чтении.

Поведение кеша закреплено юнит-тестами (`Events/EventCachingTests`, плюс два теста инвалидации в `Events/BookingConfirmedConsumerTests`): попадание, промах и инвалидация — см. [«Unit-тесты»](#unit-тесты).
...
- `TotalSeats` (`int`) — общее количество мест, должно быть больше нуля: при создании инвариант держит фабричный метод `Event.Create`, при обновлении — `[Range(1, int.MaxValue)]` в `UpdateEventDto` и та же проверка в `EventService.UpdateEventAsync` (обновление меняет поле через публичный сеттер, минуя фабрику).
