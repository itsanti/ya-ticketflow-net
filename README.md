...
- **Спринт 11**: Наблюдаемость — OpenTelemetry SDK во всех трёх сервисах (трейсы входящего/исходящего HTTP и запросов EF Core, метрики ASP.NET Core и рантайма .NET), экспорт трейсов в Jaeger по OTLP и метрик в Prometheus через `/metrics`, логи приведены к структурированному JSON через Serilog (с `TraceId`/`SpanId` в каждой записи), дашборд Grafana с latency, throughput, error rate и метриками рантайма — источник данных и дашборд подключаются через provisioning ✅
...
│
├── prometheus.yml                             # Что и как часто скрейпит Prometheus — по одному job'у на сервис
├── grafana/                                    # Конфигурация Grafana, подхватывается при старте контейнера
│   ├── provisioning/datasources/               # Источник данных Prometheus (фиксированный uid)
│   ├── provisioning/dashboards/                # Файловый провайдер дашбордов
│   └── dashboards/                             # Сам дашборд в JSON — источник истины, а не выгрузка из UI
...
└── docker-compose.yml                         # Zookeeper + Kafka + 3×PostgreSQL + Redis + Jaeger + Prometheus + Grafana + 3 сервиса — поднимаются одной командой
```
...
Телеметрия живёт в `Presentation` каждого сервиса и устроена одинаково: `DependencyInjection/ObservabilityServiceCollectionExtensions.cs` (регистрация OpenTelemetry) и `Options/OtlpOptions.cs` (имя сервиса и адрес коллектора из конфигурации), см. [«Наблюдаемость»](#-наблюдаемость-opentelemetry-prometheus-jaeger-grafana).
...
Здесь же собирается телеметрия — `AddObservability` регистрирует OpenTelemetry (трейсы и метрики), а `UseSerilog` подменяет провайдер логирования. Слой выбран не случайно: инструментация подписывается на уже существующие `DiagnosticSource`/`Meter` ASP.NET Core и EF Core, поэтому Presentation не нужна ни одна новая ссылка на Infrastructure, а Domain и Application вообще не знают, что за ними наблюдают.
...
Рядом с сервисами поднимается стек наблюдаемости — подробности в разделе [«Наблюдаемость»](#-наблюдаемость-opentelemetry-prometheus-jaeger-grafana):

| Инструмент | Роль | UI с хоста | Внутри сети Docker |
|---|---|---|---|
| **Prometheus** | Хранит метрики, сам ходит за ними к сервисам | `http://localhost:9090` | `prometheus:9090` |
| **Jaeger** | Принимает и показывает трейсы | `http://localhost:16686` | `jaeger:4317` (приём OTLP/gRPC) |
| **Grafana** | Дашборды поверх Prometheus (`admin` / `admin`) | `http://localhost:3000` | `grafana:3000` |

Метрики каждого сервиса доступны на `/metrics` — с хоста по портам из таблицы выше (например, `http://localhost:5102/metrics`), внутри сети по `<сервис>:8080/metrics`.
 ...
 **(Спринт 11)**
- [x]  OpenTelemetry SDK подключён во всех трёх сервисах одинаковым расширением `AddObservability` в `Presentation`
- [x]  Трейсы: автоматическая инструментация входящих HTTP-запросов, исходящих HTTP-запросов и запросов EF Core; экспорт в Jaeger по OTLP/gRPC
- [x]  В спанах EF Core виден текст SQL-запроса (`db.statement`), имя БД и `db.system = postgresql` — отдельной настройки для этого не требуется
- [x]  Метрики: инструментация ASP.NET Core (latency, throughput, error rate) и рантайма .NET (GC, thread pool, память), экспорт через `/metrics` в формате Prometheus
- [x]  Имя сервиса задаётся ресурсом OpenTelemetry (`events-service`/`users-service`/`bookings-service`) — по нему сервисы различаются и в Jaeger, и в Prometheus
- [x]  `/metrics` выведен из-под редиректа на https (`UseWhen`), иначе при локальном запуске по https-профилю Prometheus получал бы `307` вместо метрик
- [x]  Логи всех сервисов — структурированный JSON (Serilog + `CompactJsonFormatter`); самописный `RequestLoggingMiddleware` с `Console.WriteLine` заменён на `UseSerilogRequestLogging`
- [x]  В каждой записи лога есть `@tr`/`@sp` — идентификаторы трейса и спана, поэтому по логу можно открыть конкретный трейс в Jaeger
- [x]  Адрес коллектора и имя сервиса вынесены в конфигурацию (`OtlpOptions`, секция `Otlp`), в Docker переопределяются переменной `Otlp__Endpoint`
- [x]  Пустой `Otlp:Endpoint` отключает экспортёр трейсов — этим пользуются интеграционные тесты, чтобы не ломиться в несуществующий коллектор
- [x]  В `docker-compose.yml` добавлены Jaeger, Prometheus и Grafana; конфигурация скрейпа — в `prometheus.yml`
- [x]  Grafana настроена через provisioning: источник данных и дашборд подключаются конфиг-файлами, а не руками в UI
- [x]  Дашборд `TicketFlow — Observability` в репозитории (`grafana/dashboards/`): latency (p50/p95/p99), throughput, error rate, запросы в обработке, latency по маршрутам и метрики рантайма .NET
---
...
- **Telemetry SDK**: OpenTelemetry .NET — инструментация ASP.NET Core, HttpClient, EF Core и рантайма
- **Metrics**: Prometheus (pull-модель, скрейп `/metrics`)
- **Tracing**: Jaeger (приём по OTLP/gRPC)
- **Dashboards**: Grafana (источник данных и дашборд — через provisioning)
- **Logging**: Serilog + `Serilog.Formatting.Compact` — структурированный JSON в консоль
...
- OpenTelemetry.Instrumentation.AspNetCore / .Http / .Runtime / .EntityFrameworkCore
- OpenTelemetry.Exporter.OpenTelemetryProtocol   # трейсы в Jaeger
- OpenTelemetry.Exporter.Prometheus.AspNetCore   # эндпоинт /metrics
- Serilog.AspNetCore
- Serilog.Formatting.Compact
```

У двух пакетов OpenTelemetry стабильных релизов пока нет в принципе — `Instrumentation.EntityFrameworkCore` и `Exporter.Prometheus.AspNetCore` публикуются только как `beta`, поэтому в `Directory.Packages.props` у них предрелизные версии. Это не недосмотр, а текущее состояние экосистемы.
...
Поднимает Zookeeper, Kafka, три PostgreSQL, Redis, стек наблюдаемости и все три сервиса одной командой.
...
Отдельной команды для стека мониторинга нет — он часть того же `docker compose up`, см. [«Наблюдаемость»](#-наблюдаемость-opentelemetry-prometheus-jaeger-grafana).
...
```bash
docker compose up -d zookeeper kafka events-db redis jaeger
dotnet run --project TicketFlow.Events.Presentation
```

`jaeger` здесь нужен только если хочется смотреть трейсы: `appsettings.json` по умолчанию указывает на `http://localhost:4317`, то есть на проброшенный порт контейнера. Без него сервис работает так же, просто экспортёр периодически пишет в лог, что коллектор недоступен. Prometheus в этом варианте сервис на хосте не увидит — он скрейпит по именам контейнеров; метрики можно смотреть напрямую на `http://localhost:5102/metrics`.
...
Секретов здесь тоже нет, поэтому значения лежат в `appsettings.json` каждого сервиса:
...
`Otlp` биндится в `OtlpOptions` (Presentation): `ServiceName` становится именем ресурса OpenTelemetry, `Endpoint` — адресом коллектора трейсов. В Docker адрес переопределяется на имя контейнера — тот же приём, что с базой, Kafka и Redis:

```bash
Otlp__Endpoint: http://jaeger:4317
```

Пустой `Endpoint` — это не ошибка конфигурации, а способ выключить экспорт трейсов: экспортёр тогда просто не регистрируется. Подробности и остальные детали — в разделе [«Наблюдаемость»](#-наблюдаемость-opentelemetry-prometheus-jaeger-grafana).

Секции `Logging` в конфигурации больше нет: `UseSerilog` заменяет провайдеры логирования целиком, и `Logging:LogLevel` после этого ни на что не влияет — уровни задаются в `Serilog:MinimumLevel`.
...
Каждая фабрика дополнительно гасит экспорт трейсов — `UseSetting("Otlp:Endpoint", string.Empty)`, см. [«Телеметрия в тестах»](#телеметрия-в-тестах).
...
Сервис, о котором ничего не известно снаружи, невозможно ни отлаживать, ни планировать. С одиннадцатого спринта каждый сервис рассказывает о себе три вещи: **метрики** — что происходит в целом (сколько запросов, как долго, сколько ошибок), **трейсы** — что случилось с одним конкретным запросом и куда ушло его время, **логи** — что именно сервис при этом делал. Собирает все три сигнала один SDK — OpenTelemetry, — а хранят и показывают их разные инструменты.

![Дашборд Grafana с метриками трёх сервисов](docs/grafana-dashboard.png)

### Кто за что отвечает

| Инструмент | Роль | UI с хоста | Внутри сети Docker |
|---|---|---|---|
| **OpenTelemetry SDK** | Собирает трейсы и метрики внутри процесса | — | — |
| **Jaeger** | Принимает трейсы по OTLP, хранит и показывает | `http://localhost:16686` | `jaeger:4317` (OTLP/gRPC) |
| **Prometheus** | Скрейпит `/metrics` у сервисов и хранит метрики | `http://localhost:9090` | `prometheus:9090` |
| **Grafana** | Дашборды поверх Prometheus | `http://localhost:3000` (`admin` / `admin`) | `grafana:3000` |
| **Serilog** | Пишет логи в консоль структурированным JSON | `docker compose logs <сервис>` | — |

Весь стек поднимается тем же `docker compose up --build`, что и раньше, — отдельной команды для мониторинга нет.

**Порядок старта.** У каждого сервиса есть healthcheck (`curl` на собственный `/metrics`), а Prometheus объявлен зависимым от их готовности через `condition: service_healthy`. Без этого он начинал скрейпить раньше, чем сервисы успевали подняться, и первые минуты в Status → Targets висели красные таргеты — картина, неотличимая от настоящей поломки конфигурации. Ради этого healthcheck'а в runtime-образ ставится `curl`: в `mcr.microsoft.com/dotnet/aspnet` HTTP-клиента нет.

Зависимость сервисов от Jaeger намеренно осталась `service_started`: экспортёр трейсов спокойно переживает недоступный коллектор и копит спаны в буфере, так что ждать готовности нечего, а для полноценного healthcheck'а пришлось бы тащить probe-бинарник ещё и в образ Jaeger.

### Push и pull — почему адрес коллектора нужен только трейсам

Два сигнала доставляются противоположными способами, и это объясняет всю конфигурацию:

- **Трейсы — push.** Приложение само отправляет спаны на OTLP-эндпоинт. Значит, сервису нужно знать адрес приёмника — отсюда `Otlp:Endpoint`. Spans копятся в буфере и уходят пачками, поэтому в Jaeger они появляются с задержкой в несколько секунд.
- **Метрики — pull.** Prometheus сам ходит к сервису на `/metrics` по расписанию (`scrape_interval: 15s`). Сервису знать о Prometheus нечего — он просто держит эндпоинт открытым. Зато Prometheus должен знать адреса всех сервисов, и это `prometheus.yml`.

Из pull-модели следует неочевидное: метрик `http_server_*` не существует, пока не пришёл первый запрос — счётчики создаются при первом обращении. Пустой `/metrics` на свежеподнятом сервисе — это норма, а не поломка.

### Трейсы в Jaeger

Инструментация автоматическая, руками не размечено ни одного спана:

- `AddAspNetCoreInstrumentation` — входящий HTTP-запрос, корневой спан с методом, маршрутом и статусом;
- `AddHttpClientInstrumentation` — исходящие HTTP-вызовы;
- `AddEntityFrameworkCoreInstrumentation` — каждый запрос к БД отдельным дочерним спаном.

В спане EF Core видны `db.system = postgresql`, имя базы и **текст SQL-запроса** (`db.statement`) — например, у `GET /events` внутри корневого спана лежат два обращения к `events-db`: подсчёт общего количества и выборка страницы. Отдельно включать это не нужно: в актуальных версиях инструментации текст запроса пишется всегда, а настройка `SetDbStatementForText`, которая управляла этим раньше, из API удалена. Значения параметров запроса в спан не попадают — они за экспериментальным флагом `OTEL_DOTNET_EXPERIMENTAL_EFCORE_ENABLE_TRACE_DB_QUERY_PARAMETERS`, потому что могут содержать пользовательские данные.

Имя сервиса в Jaeger берётся из ресурса OpenTelemetry (`Otlp:ServiceName`), поэтому в выпадающем списке три различимых сервиса, а не три одинаковых `unknown_service`.

**Скрейп `/metrics` в трейсы не попадает.** Prometheus стучится к каждому сервису раз в 15 секунд, и без фильтра в Jaeger накапливались бы сотни одинаковых спанов `GET /metrics`, в которых нет ни бизнес-смысла, ни диагностической ценности — зато они разбавляют настоящие запросы и раздувают хранилище. Отсекаются они `options.Filter` в `AddAspNetCoreInstrumentation`: инструментация не создаёт активность, если функция вернула `false`. В логах то же самое сделано иначе — там скрейп не отбрасывается, а понижается до уровня `Debug`, потому что факт обращения иногда полезно увидеть при отладке самого скрейпинга.

**Чего в трейсах сознательно нет.** Сквозного трейса «Bookings → Events» не существует: сервисы общаются через Kafka, а контекст трассировки в сообщениях не передаётся. Чтобы бронирование и списание места оказались в одном трейсе, пришлось бы класть `traceparent` в заголовки сообщения у издателя и восстанавливать его у подписчика. Это выходит за рамки спринта и добавило бы в контракт обмена поле, которое к предметной области отношения не имеет, — поэтому спаны каждого сервиса живут своими трейсами.

### Метрики и `/metrics`

Собираются две группы:

- **ASP.NET Core** — `http_server_request_duration_seconds` (гистограмма длительностей, из неё считаются квантили), её же `_count` (отсюда RPS и доля ошибок) и `http_server_active_requests` (сколько запросов в обработке прямо сейчас). Лейблы `http_route`, `http_request_method`, `http_response_status_code` позволяют резать любую из них по маршруту, методу и статусу.
- **Рантайм .NET** — GC по поколениям, пул потоков, working set, счётчик исключений.

С именами рантайм-метрик есть тонкость. Начиная с .NET 9 рантайм отдаёт метрики сам, и `AddRuntimeInstrumentation()` на таких таргетах ничего не считает — он просто подписывается на встроенный меттер `System.Runtime`. Поэтому имена здесь новые, `dotnet_*`, а не старые `process_runtime_dotnet_*` из документации пакета:

| Метрика | Что показывает | Тип |
|---|---|---|
| `dotnet_gc_collections_total` | Сборки мусора, лейбл `gc_heap_generation` | настоящий счётчик — нужен `rate()` |
| `dotnet_thread_pool_thread_count_total` | Сколько потоков в пуле **сейчас** | уровень, `rate()` даст бессмыслицу |
| `dotnet_thread_pool_queue_length_total` | Длина очереди пула **сейчас** | уровень |
| `dotnet_process_memory_working_set_bytes` | Резидентная память процесса | уровень |
| `dotnet_exceptions_total` | Брошенные исключения, лейбл `error_type` | настоящий счётчик |

Суффикс `_total` у двух метрик пула потоков выглядит странно — по смыслу это не накопленная сумма, а текущее значение. Так вышло потому, что экспортёр Prometheus именует метрику по типу инструмента в SDK, а не по её смыслу. Практический вывод один: в запросах к ним `rate()` применять нельзя, берётся сырое значение.

Приятный побочный эффект от `dotnet_exceptions_total`: доменные исключения приложения (`NotFoundException` и прочие) попадают в метрики сами, без единой строчки кода — всплеск бизнес-ошибок виден на графике.

**`/metrics` и редирект на https.** В контейнере `UseHttpsRedirection` не подключается вовсе — сервис слушает только http, редиректить некуда, а middleware при старте пишет предупреждение `Failed to determine the https port for redirect`. Признак окружения — переменная `DOTNET_RUNNING_IN_CONTAINER`, которую выставляют сами официальные образы .NET.

При локальном запуске редирект остаётся, но `/metrics` выведен из-под него через `UseWhen`: иначе обычный http-запрос за метриками получил бы `307` вместо данных. Переносом вызова `MapPrometheusScrapingEndpoint` это не лечится — в minimal hosting эндпоинты выполняются терминальным middleware, то есть уже после всего пайплайна, и порядок вызовов `Map*` на редирект не влияет.

### Дашборд и provisioning

Дашборд лежит в репозитории — `grafana/dashboards/ticketflow-observability.json` — и подключается через provisioning, вместе с источником данных. Руками в UI ничего добавлять не нужно: Grafana читает конфиги из `grafana/provisioning/` при старте, а файловый провайдер перечитывает папку с дашбордами каждые 30 секунд. Из этого следует важное: **файл в репозитории — источник истины**, правки мышкой в UI при следующем цикле перезапишутся.

У источника данных зафиксирован `uid: prometheus`, и JSON дашборда ссылается именно на него. Без этого после каждого пересоздания контейнера панели искали бы источник по случайному uid и показывали «Datasource not found».

Панели: throughput (RPS), latency p50/p95/p99, error rate, запросы в обработке, latency p95 с разбивкой по маршрутам и строка метрик рантайма. Переменная **Сервис** вверху фильтрует по `job`, так что один дашборд обслуживает все три сервиса сразу или каждый по отдельности.

Два решения, которые стоит пояснить:

- **Error rate — доля, а не количество.** Панель показывает отношение ошибок ко всем запросам. Абсолютный счётчик ошибок растёт вместе с трафиком и сам по себе ни о чём не говорит: сто ошибок на миллион запросов и сто на тысячу — разные ситуации. Ряды 5xx и 4xx разделены и покрашены по-разному: первые означают сломанный сервис, вторые — отказ по вине клиента (невалидный запрос, нет прав, не найдено), и смешивать их в одну «долю ошибок» значит поднимать тревогу на пустом месте.
- **Знаменатель обёрнут в `clamp_min`, числитель — в `or ... * 0`.** Без этого панель ведёт себя странно в самом частом для учебного стенда случае — когда трафика нет: деление нуля на ноль даёт `NaN` и разрыв в графике, а если ошибок нужного класса не было ни разу, ряд отсутствует целиком. С этими обёртками при простое рисуется честный ноль.
- **Из разбивки по маршрутам исключён `/metrics`.** Скрейп-эндпоинт инструментируется наравне с бизнес-маршрутами и, будучи самым «толстым» ответом, занимал верхние строчки таблицы, вытесняя реальные эндпоинты.

### Логи

Serilog подключён как провайдер логирования и пишет в консоль `CompactJsonFormatter`'ом — каждая строка самостоятельный JSON-объект:

```json
{"@t":"2026-09-21T13:44:54.05Z","@mt":"HTTP {RequestMethod} {RequestPath} responded {StatusCode} in {Elapsed:0.0000} ms",
 "@tr":"f49cc6e85a3232f4dacbca100358664f","@sp":"691fc83ec165c3d2",
 "RequestMethod":"GET","RequestPath":"/events","StatusCode":200,"Elapsed":3.32,
 "SourceContext":"Serilog.AspNetCore.RequestLoggingMiddleware"}
```

Ключевое здесь — `@tr` и `@sp`: идентификаторы трейса и спана, которые Serilog берёт из `Activity.Current` сам, без нашего участия. Значение `@tr` можно вставить в поиск Jaeger и открыть ровно тот запрос, о котором строка лога, — со всеми его SQL-спанами. Это и есть смысл «приведения логов к единому формату»: параметры лежат отдельными полями, по которым можно фильтровать в любом сборщике, а не растворены в тексте сообщения.

**Порядок middleware здесь важен.** `UseSerilogRequestLogging` стоит **снаружи** `GlobalExceptionHandlingMiddleware`, а не внутри. Если поменять их местами, доменное исключение по пути наверх сначала попадёт в Serilog — тот увидит непойманное исключение и запишет `responded 500` со стектрейсом, — и только потом обработчик превратит его в честный `401`. Клиент получит правильный ответ, а лог будет рассказывать про несуществующую ошибку сервера. При текущем порядке к моменту возврата в Serilog ответ уже сформирован, и в лог идёт тот статус, который реально ушёл клиенту. Настоящие 500 при этом поднимаются до уровня `Error` по условию `StatusCode >= 500`, а стектрейс неожиданной ошибки пишет сам `GlobalExceptionHandlingMiddleware` через `ILogger`.

**Не всё, что попадает в консоль, проходит через Serilog.** Npgsql при старте пробует подгрузить GSSAPI (механизм аутентификации Kerberos), и если библиотеки в образе нет, нативный загрузчик .NET пишет в stderr строку `Cannot load library libgssapi_krb5.so.2` — в обход логгера, то есть не JSON. На работу это не влияет (подключение к Postgres идёт по паролю), но одна такая строка ломает правило «каждая строка — валидный JSON», на которое опирается сборщик логов. Поэтому в runtime-слой `Dockerfile` ставится пакет `libgssapi-krb5-2`: библиотека находится, загрузчик молчит, поток логов остаётся однородным.

Самописный `RequestLoggingMiddleware` из первых спринтов удалён: он писал через `Console.WriteLine` и был бы единственной не-JSON строкой в общем потоке. Его заменил `UseSerilogRequestLogging` — одна структурированная запись на запрос вместо двух текстовых. Логирование запросов вынесено из блока `if (app.Environment.IsDevelopment())`: в Docker окружение тоже `Development`, но полагаться на это для основного канала логов неправильно. Скрейп `/metrics` при этом понижен до уровня `Debug` — иначе Prometheus раз в 15 секунд писал бы по строке в лог каждого сервиса; ошибки и ответы 5xx, наоборот, подняты до `Error`.

### Телеметрия в тестах

Интеграционные тесты поднимают настоящий `Program` через `WebApplicationFactory`, то есть вместе с ним поднимали бы и экспортёр трейсов — который фоном ломился бы в несуществующий коллектор на `localhost:4317`. Поэтому в каждой `CustomWebApplicationFactory` стоит `UseSetting("Otlp:Endpoint", string.Empty)`: пустой адрес означает «экспортёр не регистрировать». Метрики при этом остаются включёнными — они никуда не отправляются и никому не мешают.

### Если что-то не видно

| Симптом | Причина |
|---|---|
| `/metrics` почти пустой | Не было ни одного запроса — счётчики создаются при первом обращении |
| Таргет в Prometheus `DOWN` | Сервис ещё поднимается, либо в `prometheus.yml` указан порт хоста вместо внутреннего `8080` |
| В Jaeger нет сервиса | Спаны уходят пачками — подождите несколько секунд после первого запроса |
| Панели «No data», хотя запрос в Prometheus работает | Не совпал `uid` источника данных с тем, что записан в JSON дашборда |
| Ссылки в Prometheus → Targets не открываются | Так и должно быть: там URL с точки зрения Prometheus внутри сети Docker. С хоста метрики смотрите по `http://localhost:5102/metrics` |
| Grafana показывает старый дашборд | `docker compose down -v` очистит том `grafana-data` |
...
**Что при этом видно снаружи.** Каждый HTTP-шаг сценария попадает в Jaeger отдельным трейсом — с дочерними спанами запросов к БД и текстом SQL, — в метрики на дашборде Grafana и в JSON-лог с тем же `TraceId`, что у трейса. Единого сквозного трейса на весь сценарий нет: шаги 3–4 идут через Kafka, а контекст трассировки в сообщениях не передаётся, см. [«Наблюдаемость»](#-наблюдаемость-opentelemetry-prometheus-jaeger-grafana).
