...
**`/metrics` и редирект на https.** Эндпоинт выведен из-под `UseHttpsRedirection` через `UseWhen`. Причина: при локальном запуске по https-профилю обычный http-запрос на `/metrics` получал бы `307` вместо метрик. Переносом вызова `MapPrometheusScrapingEndpoint` эта проблема не решается — в minimal hosting эндпоинты выполняются терминальным middleware, то есть уже после всего пайплайна, и порядок вызовов `Map*` на редирект не влияет.
