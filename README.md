...
- **Error rate — доля, а не количество.** Панель показывает отношение 5xx (и отдельно 4xx) ко всем запросам. Абсолютный счётчик ошибок растёт вместе с трафиком и сам по себе ни о чём не говорит: сто ошибок на миллион запросов и сто на тысячу — разные ситуации.
